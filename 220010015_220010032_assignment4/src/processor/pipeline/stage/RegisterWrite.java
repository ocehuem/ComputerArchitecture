	...
	public void performRW() {
		if (MA_RW_Latch.isRW_enable()) {
			if (!MA_RW_Latch.isBubbled()) {
				...
				// if instruction being processed is a load instruction, remember to set the
				// corresponding register in the register file

				// if instruction being processed is a store instruction, remember to write the
				// value to the memory location

				// if instruction being processed is a branch instruction, remember to set the
				// PC of the processor
				...
				if (opcode == 29) {
					Simulator.setISsimulationComplete(true);

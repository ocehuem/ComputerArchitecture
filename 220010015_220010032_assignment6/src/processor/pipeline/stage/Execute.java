			...
			if (!OF_EX_Latch.isBubbled()) {
				...
				if (opcode >= 0 && opcode <= 28) {
					...
					if (opcode == 4 || opcode == 5) {
						// Multiplication
						event = new ExecutionCompleteEvent(Clock.getCurrentTime() + Configuration.multiplier_latency,
								this, this);
					} else if (opcode == 6 || opcode == 7) {
						// Division
						event = new ExecutionCompleteEvent(Clock.getCurrentTime() + Configuration.divider_latency, this,
								this);
					} else {
						// ALU
						event = new ExecutionCompleteEvent(Clock.getCurrentTime() + Configuration.ALU_latency, this,
								this);
					}
